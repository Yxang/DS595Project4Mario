...
class Agent_DDQN():
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            torch.save(self.model.state_dict(), f'{save_prefix}_DDQN/DDQN_model_ep{epoch}.pt')
            pickle.dump(rewards, open(f"{save_prefix}_DDQN/DDQN_reward_ep{epoch}.pkl", 'wb'))
            pickle.dump(flag, open(f"{save_prefix}_DDQN/flag_ep{epoch}.pkl", 'wb'))
            plt.savefig(f"{save_prefix}_DDQN/epoch{epoch}.png")
            learn_curve = []
